...
Cell nuclei are tracked across a tif stack. The trace is represented in the
pase contrast stack.
Traces and information about the lines are saved in folder 'tracks'.
...
% Alexa 647 batch 06/20
%Unikon bin2: 156
%UNikon no bin: 41
%TIRF bin2: 127
%TIRF no bin: 32

% Alexa 647 batch 02/21
%UNikon bin2: 81 (measured 04/21)
%UNikon no bin: 21 (measured 04/21)
%TIRF bin2: 122 (measured 04/21)
%TIRF no bin: 206 (measured 04/21)

int2dens = 1/156; %UNikon, 10x, bin2; Alexa 647 batch 06/20
int2dens = 1/81; %UNikon, 10x, Cy5_bin2 filter; Alexa 647 batch 02/21
...
%BWtreshhold=0.04;% used for all the Unikon data (between 0.005 & 0.15)
%BWtreshhold= 1;% 0.1 used for all the Unikon data with SiR Hoechst(between 0.005 & 0.15)
% BWthreshold= 0.1;% 0.1 used for all the Unikon data with H2B mCherry(between 0.005 & 0.15) if 8-bit
BWthreshold= 0.01;% Lat A experiments October 2021

%BWtreshhold= 1;% ? used for all the TIRF data with H2B mCherry(between 0.005 & 0.15)

%% variables to filter out tracks
% mincelldistance = 50/pixelsize;% minimum distance between neighbouring cells [um]
...
try
    for filenr = startpoint:size(FileList2,1)
        ...
        for k = 1:1:num_images
            ...
            %         % calculate center and mass of each white feature
            %         s = regionprops(im_thres,'centroid','area');
            %         ind=1;
            %         xypos=[];
            %         for i=1:size(s,1)
            %             if s(i).Area>minarea
            %                 xypos(ind,:)=s(i).Centroid;
            %                 ind=ind+1;
            %             end
            %         end
            %         clear ind
